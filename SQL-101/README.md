...
Можно добавить `PERCENT` - тогда это будет число процентов от общего количества строк. Можно добавить `WITH TIES` - в этом случае отразятся все повторяющиеся значения, попадающие в выборку (строк будет больше, чем задано после `TOP`)
...
Два типа планов:
 - Предполагаемый план: анализ запроса до его исполнения
 - Фактический план: отображение фактически пройденных шагов при исполнении запроса

 Форматы отображения запросов:
  - Графический
  - Текстовый
  - XML
Ниболее читаемый из их - графический формат

## Типы физических соединений
 - Nested Loops или соединения вложенных циклов
 - Merge Joints или соединения слиянием
 - Hash Joints или хещ-соединения

### Nested Loops
![nested loops](https://github.com/Bupley/datalearn/blob/main/SQL-101/nested_loops.PNG)

Справа вверху - внешний ввод, справа внизу - внутренний ввод. Этот оператор для каждой записи из внешнего ввода находит совпадающие значения из внутреннего ввода. Применяется когда внешний ввод относительно небольшой,а  внутренний - индексирова и большого объема.

### Merge Joints
![merge](https://github.com/Bupley/datalearn/blob/main/SQL-101/merge.PNG)

выбирается в случаях:
1. Наличия сортированных (clustered) и несориторванных (nonclustered) индексов
2. Большое количество возвращаемых строк
3. Использован оператор равенства

### Hash Joints
![hash](https://github.com/Bupley/datalearn/blob/main/SQL-101/hash.PNG)
Используется, когда данные не отсоритрованы, т.е. в них нет индексов. 

## Индексы
Индекс - структура, занмающая определнное место в памяти, связанная с таблицей или представлением и созданная для ускорения извлеченяи строк. Индекс содержит в себе ключи, которые могут состоять из одного столбца или нескольких. Эти ключи хранятся в структуре бинарного дерева:

![b-tree](https://github.com/Bupley/datalearn/blob/main/SQL-101/B-tree.PNG)

### Кластерные индексы
Если в таблице есть кластерный индекс, то эта таблица называется кластерной: в ней строки отсортированы по кластерному индексу. Такая таблица обеспечивает быстрый доступ к данным, но требуется дополниельное вермя на пересортировку при добавлении/удалении строк, и требуеутся больше места для хранения дерева индексов.  
Кластерные индексы должны быть как можно меньше по размеру, уникальными и неизменящимися. Кластерный индекс можетбыть только один, и обцчно он совпадает с PRIMARY KEY.

### Некластерные индексы
Используются для для столбца, по которому будут выполняться операции JOIN, WHERE, ORDER BY. Может создаваться несколько индексов. Если в таблице нет индексов или есть только некластерные индексы, то это некластерная таблица (или heap table, куча). Из нее дольше извлексать данные, но не требуется дополнительное время для пересортировки и дополнительное место для хранения.
