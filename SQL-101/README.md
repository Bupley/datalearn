...
```
После `CREATE TABLE` указывается название новой таблицы, далее в скобках через запятую перечисляются названия столбцов и тип данных.

### Создание таблицы н основе данных в уже существующей
```
SELECT
	[AddressID],
	[AddressLine1],
