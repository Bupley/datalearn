...
Не совсем понятно, как представдения защищают от измененияданных, н опри этом есть команды, позволяющие менять исходные данные в таблицах!
```
UPDATE
...
В данном запросе между `BEGIN` и `END` прописан составной запрос, который с помощью этих команд заключен в определенный блок. Этот же запрос можно записать через AFTER-триггер, но нужно полсе строчки `PRINT ...` добавить `ROLLBACK TRANSACTION` - эта команда отменит выполненную процедуру.
